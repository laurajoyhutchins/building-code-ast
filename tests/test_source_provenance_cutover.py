...
class ProvenanceStranglerCutoverTests(unittest.TestCase):
    def test_canonical_source_packages_are_the_only_committed_source_authority(self) -> None:
        packages = {
            ...
            for path in CORPORA.glob("*/source-package.json")
        }
        ...
        for corpus, path in sorted(packages.items()):
            with self.subTest(corpus=corpus):
                ...
                self.assertEqual(
                    source_package_from_dict(json.loads(path.read_text(encoding="utf-8"))).to_dict(),
                    package.to_dict(),
                )
    ...
    def test_source_index_is_a_deterministic_projection_of_packages(self) -> None:
        ...
        generated = build_source_index(packages)
        committed = json.loads((CORPORA / "source-index.json").read_text(encoding="utf-8"))
        self.assertEqual(committed, generated)

    def test_legacy_authority_modules_are_deleted(self) -> None:
        evidence = ROOT / "src" / "building_code_ast" / "evidence"
        for name in ("io.py", "model.py", "source_objects.py", "source_object_hydration.py"):
