import unittest

from building_code_ast.aisc360_raster_hierarchy_observation import (
    AISC360_DERIVATIVE_SHA256,
    AISC360_REPRESENTATIVE_RENDER_RECIPE,
    RasterHierarchyPageObservation,
    summarize_raster_hierarchy_observations,
)


def _observation(
    *,
    page: int,
    text: str,
    source_sha256: str = AISC360_DERIVATIVE_SHA256,
    renderer: str = "pdftoppm",
    renderer_version: str = "25.06.0",
    output_format: str = "png",
):
    return RasterHierarchyPageObservation(
        page_number=page,
        source_derivative_sha256=source_sha256,
        render_sha256="a" * 64,
        render_dpi=600,
        render_renderer=renderer,
        render_renderer_version=renderer_version,
        render_output_format=output_format,
        recovery_backend="source-safe-test",
        recovered_text=text,
    )


class TestAisc360RasterHierarchyObservation(unittest.TestCase):
    def test_raster_observation_emits_source_safe_locator_evidence(self) -> None:
        result = summarize_raster_hierarchy_observations(
            [_observation(page=243, text="1.3. SYNTHETIC HEADING\nsynthetic body")]
        )

        self.assertEqual(
            result,
            {
                "schema": "aisc360-raster-hierarchy-observation-v1",
                "source_derivative_sha256": AISC360_DERIVATIVE_SHA256,
                "render_recipe": AISC360_REPRESENTATIVE_RENDER_RECIPE,
                "observations": [
                    {
                        "page": 243,
                        "source_kind": "raster_recovery",
                        "render_sha256": "a" * 64,
                        "render_recipe": AISC360_REPRESENTATIVE_RENDER_RECIPE,
                        "recovery_backend": "source-safe-test",
                        "recovered_text_sha256": "3d727c41e458c3a07cad471106bf66e37916644d9ac5a72ceab65f9fc3628e24",
                        "dotted_hierarchy_locators": ["1.3"],
                    }
                ],
                "parser_promotion_performed": False,
            },
        )
        self.assertNotIn("SYNTHETIC HEADING", repr(result))
        self.assertNotIn("synthetic body", repr(result))

    def test_raster_observation_rejects_wrong_source_identity(self) -> None:
        with self.assertRaisesRegex(ValueError, "source derivative"):
            summarize_raster_hierarchy_observations(
                [_observation(page=243, text="1.3. SYNTHETIC", source_sha256="b" * 64)]
            )

    def test_raster_observation_does_not_infer_single_level_numbering(self) -> None:
        result = summarize_raster_hierarchy_observations(
            [_observation(page=285, text="3. SYNTHETIC CHILD\n4.2. SYNTHETIC SECTION")]
        )

        self.assertEqual(result["observations"][0]["dotted_hierarchy_locators"], ["4.2"])

    def test_raster_observation_requires_recovery_provenance(self) -> None:
        with self.assertRaisesRegex(ValueError, "render_|recovery_backend"):
            RasterHierarchyPageObservation(
                page_number=243,
                source_derivative_sha256=AISC360_DERIVATIVE_SHA256,
                render_sha256="a" * 64,
                render_dpi=0,
                render_renderer="",
                render_renderer_version="",
                render_output_format="",
                recovery_backend="",
                recovered_text="1.3. SYNTHETIC",
            )

    def test_raster_observation_rejects_render_recipe_drift(self) -> None:
        for kwargs in (
            {"renderer": "other-renderer"},
            {"renderer_version": "25.05.0"},
            {"output_format": "jpeg"},
        ):
            with self.subTest(kwargs=kwargs):
                with self.assertRaisesRegex(ValueError, "render recipe"):
                    summarize_raster_hierarchy_observations(
                        [_observation(page=243, text="1.3. SYNTHETIC", **kwargs)]
                    )


if __name__ == "__main__":
    unittest.main()
