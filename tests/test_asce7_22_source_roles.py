...
def test_bold_9_5_normative_candidate_is_explicit_declaration_context() -> None:
    ...
    assert selected[0].section_declaration is True


def test_regular_numeric_leading_candidate_fails_closed_as_non_declaration() -> None:
    ...
    assert selected[0].section_declaration is False


def test_bold_italic_candidate_fails_closed_as_non_declaration() -> None:
    # Exact-source collision family 7.1.2: page 235 is bold-italic 9.5 pt.
    lookalike = _qualified(
        _observation("7.1.2 SYNTHETIC ROLE-DISTINCT HEADING", 235, font="Synthetic.BI", size=9.5)
    )
...
def test_numeric_candidate_without_span_evidence_fails_closed() -> None:
...
def test_declaration_context_suppresses_numeric_lookalike_section_promotion() -> None:
    chapter = _qualified(
        _observation(
            "CHAPTER 7 SYNTHETIC",
            178,
    ...
    assert ast.root.children[0].node_type is DocumentNodeType.CHAPTER
    assert ast.root.children[0].children[0].node_type is DocumentNodeType.PARAGRAPH
