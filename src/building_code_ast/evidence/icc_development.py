"""Bounded adapters for official ICC proposal monographs and action reports."""
...
_ACTION_LINE_RE = re.compile(r"^Committee Action:\s*(?P<action>.+)$", re.IGNORECASE)
...
def _action_disposition(value: str) -> DevelopmentDisposition | None:
    ...
    if normalized.startswith("as submitted") or normalized.startswith("approved as submitted"):
        return DevelopmentDisposition.APPROVED
    if normalized.startswith("as modified") or normalized.startswith("approved as modified"):
...
class IccCommitteeActionPdfAdapter:
    """Extract action records from an official ICC committee-action report."""

    adapter_id = "icc-committee-action-pdf"
    adapter_version = "0.1.0"
    supported_roles = frozenset({EvidenceRole.DEVELOPMENT_HISTORY})
    supported_media_types = frozenset({"application/pdf"})
    ...
    def __init__(
        ...
        *,
        ...
        for proposal_id, locators in affected_locators_by_proposal.items():
            if _PROPOSAL_HEADING_RE.fullmatch(proposal_id) is None:
        ...
        for page_number, lines in pages:
            for index, line in enumerate(lines):
                ...
                for candidate in lines[index + 1 : index + 5]:
                    action_match = _ACTION_LINE_RE.fullmatch(candidate)
