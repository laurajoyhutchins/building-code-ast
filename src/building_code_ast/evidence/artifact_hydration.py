"""Verified hydration for exact artifacts independent of logical source IDs."""
...
class ArtifactHydrationReceipt:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "type": "artifact_hydration_receipt",
...
def _verify_path(path: Path, artifact: Artifact) -> None:
    ...
    try:
        ...
        if before.st_size != artifact.size:
            raise ValueError(
                f"artifact size mismatch: expected {artifact.size}, observed {before.st_size}"
            )
    ...
    if observed != artifact.sha256:
        raise ValueError(
            f"artifact sha256 mismatch: expected {artifact.sha256}, observed {observed}"
        )
...
def verify_local_artifact(
    ...
    """Verify already-local bytes against canonical artifact identity."""
...
def hydrate_artifact(
    ...
    *,
    ...
    """Fetch, verify, and atomically place one exact private artifact."""
    if not isinstance(artifact, Artifact):
        raise TypeError("artifact must be an Artifact")
    ...
    fd, temporary_name = tempfile.mkstemp(
        prefix=f".{destination_path.name}.", suffix=".partial", dir=parent
    )
    os.close(fd)
