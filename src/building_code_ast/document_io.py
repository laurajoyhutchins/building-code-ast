...
from .document_model import DOCUMENT_AST_VERSION, DocumentAst, DocumentNode, DocumentNodeType, DocumentSourceArtifact
...
def _span(value: Any, label: str) -> SourceSpan:
    ...
    return SourceSpan(start=start, end=end, text=_string(obj["text"], f"{label}.text"))
...
def _node(value: Any, path: str) -> DocumentNode:
    ...
    _exact_keys(obj, {"node_id", "type", "locator", "span", "label", "attributes", "children"}, path)
    try:
    ...
    for name in sorted(attributes_obj):
        ...
        attributes.append((name, _string(attributes_obj[name], f"{path}.attributes[{name!r}]")))
    children_value = obj["children"]
    if not isinstance(children_value, list):
        raise ValueError(f"{path}.children must be an array")
    children = tuple(_node(child, f"{path}.children[{index}]") for index, child in enumerate(children_value))
    return DocumentNode(
...
def document_ast_from_dict(value: Mapping[str, Any]) -> DocumentAst:
    ...
    _exact_keys(obj, {"ast_version", "type", "source_text", "source_artifact", "root", "diagnostics"}, "document AST")
    ...
    source_artifact = DocumentSourceArtifact(
        ...
        publication_component_id=(
            _string(artifact_obj["publication_component_id"], "source_artifact.publication_component_id")
    ...
    ast = DocumentAst(
        ...
        diagnostics=tuple(_diagnostic(diagnostic, index) for index, diagnostic in enumerate(diagnostics_value)),
    )
