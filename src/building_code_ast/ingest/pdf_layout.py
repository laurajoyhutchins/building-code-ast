...
class PdfSpan:
    ...
    def to_dict(self) -> dict[str, object]:
        return {"bbox": [round(value, 3) for value in self.bbox], "text": self.text, "font": self.font, "size": round(self.size, 3), "flags": self.flags}
...
class PdfLine:
    ...
    def to_dict(self) -> dict[str, object]:
        payload: dict[str, object] = {"bbox": [round(value, 3) for value in self.bbox], "spans": [span.to_dict() for span in self.spans]}
...
class PdfBlock:
    ...
    def to_dict(self) -> dict[str, object]:
        payload: dict[str, object] = {"page_number": self.page_number, "bbox": [round(value, 3) for value in self.bbox], "text": self.text, "block_number": self.block_number}
...
def order_content_blocks(blocks: Iterable[PdfBlock], page_width: float, *, top_content_y: float = 65.0, bottom_content_y: float = 730.0) -> tuple[PdfBlock, ...]:
    """Filter recurring headers/footers and return two-column reading order."""
    midpoint = page_width / 2.0
    retained = [block for block in blocks if block.text.strip() and block.bbox[1] >= top_content_y and block.bbox[3] <= bottom_content_y]
...
def _table_region_bboxes(page: object, block_texts: Iterable[str]) -> tuple[tuple[float, float, float, float], ...]:
    ...
    return tuple(tuple(float(value) for value in table.bbox) for table in tables)


def _table_region_id(bbox: tuple[float, float, float, float], regions: tuple[tuple[float, float, float, float], ...]) -> int | None:
    ...
    for raw_block in raw.get("blocks", ()):
        ...
        for raw_line in raw_block.get("lines", ()):
            ...
            for raw_span in raw_line.get("spans", ()):
                ...
                spans.append(PdfSpan(bbox=bbox, text=str(raw_span.get("text", "")), font=str(raw_span.get("font", "")), size=float(raw_span.get("size", 0.0)), flags=int(raw_span.get("flags", 0))))
            line_bbox = tuple(float(value) for value in raw_line.get("bbox", (0, 0, 0, 0)))
            lines.append(PdfLine(bbox=line_bbox, spans=tuple(spans), direction=_line_direction(raw_line)))
...
def extract_pdf_layout(path: Path | str) -> PdfLayoutDocument:
    ...
    try:
        ...
        raise RuntimeError("PyMuPDF is required for PDF ingestion; install building-code-ast[nec-pdf]") from exc
    source = Path(path)
    ...
    try:
        ...
        for page_index in range(document.page_count):
            ...
            table_regions = _table_region_bboxes(page, (str(block[4]) for block in raw_blocks if len(block) > 4))
            ...
            for block in raw_blocks:
                ...
                blocks.append(PdfBlock(page_number=page_index + 1, bbox=bbox, text=str(text), block_number=block_number, table_region_id=_table_region_id(bbox, table_regions), lines=line_evidence.get(block_number, ())))
            pages.append(PdfPage(page_number=page_index + 1, width=float(page.rect.width), height=float(page.rect.height), blocks=tuple(blocks)))
        outline = tuple(PdfOutlineItem(level=int(level), title=str(title), page_number=int(page_number)) for level, title, page_number in document.get_toc(simple=True))
        return PdfLayoutDocument(file_name=source.name, pages=tuple(pages), outline=outline)
    finally:
        document.close()
