...
def validate_line_coverage(retained_lines: Sequence[VisualLine], removed_lines: Sequence[RemovedLine], blocks: Sequence[Any]) -> None:
    retained_ids = [line.line_id for line in retained_lines]
    retained_counts = Counter(retained_ids)
    retained_by_id = {line.line_id: line for line in retained_lines}
    duplicate_retained = sorted(line_id for line_id, count in retained_counts.items() if count > 1)
    if duplicate_retained:
        raise ValueError("duplicate retained line ID: " + ", ".join(duplicate_retained))
    removed_ids = []
    ...
    if overlap:
        raise ValueError("line cannot be retained and removed: " + ", ".join(overlap))
    ...
    for block in blocks:
        ...
        for line_id in line_ids:
            ...
            consumed[line_id] += 1
        block_fragments = getattr(block, "fragments", None)
        if block_fragments is not None and line_ids:
            expected_fragments = Counter(
                fragment
                for line_id in line_ids
                for fragment in retained_by_id[line_id].fragments
            )
            if Counter(block_fragments) != expected_fragments:
                raise ValueError("block fragments do not match its source lines")
...
def validate_page_profiles(pages: Sequence[CleanedPage], profiles: Sequence[PageOrderProfile]) -> None:
...
def validate_table_candidate(table: TableCandidate) -> None:
    for row in table.rows:
        ...
        for cell in row.cells:
            if any(fragment not in row_fragments for fragment in cell.fragments):
                raise ValueError("table cell fragment is outside its parent row")
        cell_fragment_counts = Counter(
            fragment for cell in row.cells for fragment in cell.fragments
        )
        if cell_fragment_counts != Counter(row.fragments):
            raise ValueError("table cells do not partition their parent row fragments")
        for cell in row.cells:
            valid_bounds = (
...
def validate_layout_projection(chapter: Any) -> None:
    ...
    retained = tuple(line for page in pages for line in getattr(page, "retained", ()))
    removed = tuple(item for page in pages for item in getattr(page, "removed", ()))
